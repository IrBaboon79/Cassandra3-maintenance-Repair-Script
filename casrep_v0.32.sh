...
##### VERSION #####
script_version='v0.32'
script_touchdate='30/11/2022'
...
##  v0.31 / 09-2022   | - minor update: report after-action repair status in log, added hard-exit if not root.
##  v0.32 / 11-2022   | - for loop logic fixed; luckily it never triggered...
## --------------------------------------------------------------------------------------------------------------------------------------------------------------
## Script Explanation
...
if [ -f $LOGFILE ]; then # only if file exists..
  ...
  LOGFILESIZE=$(du -sm "$LOGFILE" | cut -f 1)
        if [ $LOGFILESIZE -lt $MAX_LOG_SIZE ]; then
                LOGSTAT="Logfile $LOGFILE size below $MAX_LOG_SIZE MiB threshold. Current size: $LOGFILESIZE MiB."
         else
                rm -f $LOGFILE
                LOGSTAT="Logfile $LOGFILE size of $LOGFILESIZE MiB exceeded $MAX_LOG_SIZE MiB threshold - logfile was cleared!"
        fi
...
log_write "Configuration: Cassandra Configuration: $CASSANDRA_CONFIG_DIR/cassandra.yaml"
log_write "Configuration: nodetool location: $CASSANDRA_NODETOOL_DIR "
log_write "Configuration: Select Repair Mode algorithm: $REP_ALGO"
...
if [[ "$Local_Listen_Address" = "$Commander_Node" ||  "$Local_Listen_Address" = "localhost" ]] ; then
    EXITCODE=0 # default exit code, all still good here.
    ...
    for Node in $Nodes_To_Repair_Today_Array; do # for each node in need of repair...
        ...
        # here we allow an exit in case a node is 100%
        if [ $NodeRepairStatus -eq 100 ]; then
            log_write "Repair status indicates no repair needed for Node $Node - skipping!"
            EXITCODE=0 # we set a 'nice' errorcode but don't hard-exit as there may be another node in the list...
        ...
        if [ $NodeRepairStatus -lt $ForceFullRepairThreshold ]; then
            ...
            # select the repair mode here
                    ...
                    # note both responses ADD the parameter 'dash' on the front...
                        weeknumber)
                                    REPAIR_OPTION="-$(weeknum_mod "${REPAIR_OPTION_FLAGS[@]}")"
                                ;;
        ...
        if [ $? -ne 0 ]; then
           ...
           EXITCODE=1 # we set a 'bad' errorcode but don't hard-exit as there may be another node in the list...note that next iteration CAN reset exitcode
           continue
        else
             ...
             if [ $EXITCODE -ne 1 ]; then # set a 'nice' exitcode unless a previous exit code was bad; but don't hard-exit as there may be another node in the list...
                EXITCODE=0
             fi
        fi
    done # end for loop

    exit_script $EXITCODE # here we are out of the for-loop so all nodes should be handled; now we can hard/clean-exit.
else
     ...
     exit_script 0 # set a 'nice' errorcode as it's a valid exit condition
fi
