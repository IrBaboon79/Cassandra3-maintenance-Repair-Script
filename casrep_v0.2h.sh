...
##### VERSION #####
script_version='v0.2h'
script_touchdate='25/08/2022'
...
if [[ "$Local_Listen_Address" = "$Commander_Node" ||  "$Local_Listen_Address" = "localhost" ]] ; then
                                                     ...
                                                     for Node in $Nodes_To_Repair_Today_Array; do # for each node in need of repair...
                                                            ...
                                                            case $NodeRepairStatus in
                                                                    [0-9]|[0-8][0-9]|9[0-3]*) # overwrite chosen method to full in cases where it's <94%!
                                                                                              log_write "Repair status below threshold (<94%), forcing Full Repair Mode!"
                                                                                              REPAIR_OPTION="-full"
                                                                                              ;; 
                                                                                           *) # if >94%, just use the algorithm to select a method by 'chance'
                                                                                               log_write "Repair Status within threshold (>94%), Selecting mode via configured algortihm." 
